...
# --- TAMBAHAN PRESET (TIDAK MENGUBAH LOGIKA ASLI) ---
...
if PRESET == "Grade D (Market Merah Cari Alpha)":
    d_tf, d_ma = "1 Jam (1H)", 20
elif PRESET == "Grade A Setup" or PRESET == "Grade B Setup":
    d_tf, d_ma = "Harian (Daily)", 50
else:
    d_tf, d_ma = "Harian (Daily)", 10
...
TF_PILIHAN = st.sidebar.selectbox(
    ...
    index=["Harian (Daily)", "1 Jam (1H)", "30 Menit (30m)", "15 Menit (15m)", "5 Menit (5m)"].index(d_tf)
)
...
MA_PERIODE = st.sidebar.selectbox(
    ...
    index=[5, 10, 20, 50, 200].index(d_ma)
)
